...
def main():
    ...
    # FREEZE Content Encoder to preserve character structure
    ...
    print(f"Dataset loaded. Size: {len(train_font_dataset)}")
    if len(train_font_dataset) == 0:
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, samples in enumerate(train_dataloader):
            ...
            if global_step % args.log_interval == 0:
                log_msg = f"[{time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))}] Global Step {global_step} => train_loss = {loss}"
                logging.info(log_msg)
                print(log_msg) # Force print to stdout for UI capture
